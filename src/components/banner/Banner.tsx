...
const Banner: React.FC = () => {
  ...
  if (error) return <div className="text-center text-red-600">Error fetching products.</div>;

  const displayedProducts = products.slice(50, 54); 
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {displayedProducts.map((product: Product) => (
          <div
            key={product.id}
            className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 relative transform hover:scale-105" // Added hover scale effect
          >
